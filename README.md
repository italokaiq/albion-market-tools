...
**Atualização depois de testar de verdade, com sua aprovação**: rodar `install.ps1` de verdade (não só validar sintaxe) achou um bug real — sem BOM UTF-8, o Windows PowerShell 5.1 (`powershell.exe`, diferente do `pwsh` usado para validar) lê o arquivo pelo codepage ANSI do sistema e corrompe qualquer acento ou travessão dentro de uma string, quebrando o parser. Corrigido reescrevendo os dois scripts com BOM; `test_installer_scripts.py` (2 testes novos) confere isso automaticamente daqui pra frente. Instalado, verificado o atalho e o registro em "Aplicativos instalados", desinstalado (confirmando que mantém os dados por padrão) e reinstalado — ciclo completo testado, não só a sintaxe.
